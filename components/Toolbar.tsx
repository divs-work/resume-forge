...
import { shell, modeGradient, modeActivePill, modeExportGlow } from "@/constants/theme";
...
export default function Toolbar({
  ...
  const selectedFontName = FONT_OPTIONS.find((f) => f.id === fontId)?.name ?? "Default Font";
          ...
          <div className="w-5 h-5 flex items-center justify-center shrink-0">
            <svg viewBox="0 0 512 512" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
              <defs>
                ...
                <clipPath id="bgClip2"><rect x="48" y="36" width="360" height="430" rx="28" /></clipPath>
              </defs>
              <g filter="url(#ds2)"><rect x="48" y="36" width="360" height="430" rx="28" fill="url(#bgDoc2)" /></g>
          ...
          {(["markdown", "latex", "html"] as EditorMode[]).map((m) => (
            <button
              ...
              className={`px-3 py-1.5 text-[11px] font-medium transition-all duration-200 border-r border-[#E0E0E0] last:border-r-0 ${
                mode === m
                  ? modeActivePill[m]
                  : `${shell.textMuted} hover:text-[#616161] hover:bg-black/[0.04]`
              }`}
            >
      ...
      {(mode === "markdown" || mode === "latex") && (
        <div className="hidden lg:flex items-center gap-2 font-mono" onClick={(e) => e.stopPropagation()}>
          ...
          <input
            ...
            className="w-10 px-1.5 h-[22px] text-[11px] text-center bg-white text-[#616161] border border-[#E0E0E0] rounded-full outline-none focus:border-[#1976D2]/60 transition-colors"
          />
          <span className="text-[10px] text-[#8c959f]">mm</span>
          <span className="w-px h-3.5 bg-black/[0.08]" />
          ...
          <input
            ...
            onKeyDown={(e) => { if (e.key === "ArrowUp" || e.key === "ArrowDown") e.preventDefault(); }}
            className="w-12 px-1.5 h-[22px] text-[11px] text-center bg-white text-[#616161] border border-[#E0E0E0] rounded-full outline-none focus:border-[#1976D2]/60 transition-colors"
          />
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="hidden md:block" data-font-dropdown onClick={(e) => e.stopPropagation()}>
          <button
            ...
            className={`flex items-center gap-1.5 px-3 h-[30px] rounded-full text-[11px] font-medium border bg-white transition-colors outline-none min-w-[130px] justify-between ${
              fontOpen
          ...
          >
            ...
            <svg
              ...
              width="10" height="10" className={`shrink-0 transition-transform duration-150 ${fontOpen ? "rotate-180" : ""}`}
            >
        ...
        {fontOpen && createPortal(
          ...
          >
            <div className="max-h-72 overflow-y-auto">
              ...
              {/* Default */}
              <button
                onClick={() => { setFontId(""); onCloseAllAction(); setFontOpen(false); }}
                className={`w-full px-3 py-1.5 text-[11px] text-left transition-colors ${
                  !fontId ? "bg-[#E3F2FD] text-[#1565C0] font-medium" : "text-[#212121] hover:bg-[#F5F5F5]"
                }`}
              >
                Default Font
              </button>
              ...
              {(["sans-serif", "display", "serif", "monospace"] as const).map((cat) => (
                <div key={cat}>
                  ...
                  {FONT_OPTIONS.filter((f) => f.category === cat).map((f) => (
                    <button
                      key={f.id}
                      onClick={() => { setFontId(f.id); onCloseAllAction(); setFontOpen(false); }}
                      className={`w-full px-3 py-1.5 text-[11px] text-left transition-colors ${
                        fontId === f.id ? "bg-[#E3F2FD] text-[#1565C0] font-medium" : "text-[#212121] hover:bg-[#F5F5F5]"
                      }`}
                    >
                      {f.name}
                    </button>
