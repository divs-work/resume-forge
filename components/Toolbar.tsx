...
function PanelBtnIcon({ panel }: { panel: SidePanel }) {
...
export default function Toolbar({
  ...
  const panelBtnCls = (p: SidePanel) =>
    `w-8 h-8 flex items-center justify-center rounded-full text-[10px] font-medium transition-all duration-200 ${
      sidePanel === p
  ...
  return (
    <header
      className={`relative z-20 flex flex-col sm:flex-row sm:items-center sm:justify-between px-3 sm:px-4 sm:h-12 shrink-0 ${shell.bg} border-b ${shell.border}`}
      style={{ backdropFilter: "blur(20px)" }}
      onClick={onCloseAllAction}
    >
      {/* ══ Row 1: always visible ══ */}
      <div className="flex items-center justify-between h-12 sm:h-auto sm:flex-1 sm:gap-3">

        {/* Logo + Divider + Mode switcher */}
        <div className="flex items-center gap-3 shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 flex items-center justify-center shrink-0">
              <svg viewBox="0 0 512 512" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
                ...
                <g filter="url(#ds2)">
                  <rect x="48" y="36" width="360" height="430" rx="28" fill="url(#bgDoc2)" />
                </g>
                ...
                <g filter="url(#fs2)">
                  <path d="M96,124 Q96,96 124,96 L358,96 L456,194 L456,468 Q456,496 428,496 L124,496 Q96,496 96,468 Z" fill="url(#fgDoc2)" />
                </g>
                ...
                <path d="M358,96 L456,194 L370,194 Q358,194 358,182 Z" fill="url(#fold2)" />
                <path d="M124,96 L358,96" stroke="#FFE0B2" strokeWidth="1.5" strokeLinecap="round" opacity="0.35" fill="none" />
                <g filter="url(#rs2)">
                  <text x="276" y="430" fontFamily="Georgia,serif" fontSize="168" fontWeight="700" fill="url(#rGrad2)" textAnchor="middle" letterSpacing="-4">R</text>
                </g>
              </svg>
            </div>
          ...
          <div className="flex items-center border border-[#E0E0E0] rounded-full overflow-hidden bg-white">
            {(["markdown", "latex", "html"] as EditorMode[]).map((m) => (
              <button
                ...
                className={`px-2 sm:px-3 py-1.5 text-[11px] font-medium transition-all duration-200 border-r border-[#E0E0E0] last:border-r-0 ${
                  mode === m ? modeActivePill[m] : `${shell.textMuted} hover:text-[#616161] hover:bg-black/4`
                }`}
              >
          ...
          {/* Editor/Preview toggle — sm to lg only */}
          <div className="hidden sm:flex lg:hidden items-center border border-[#E0E0E0] rounded-full overflow-hidden bg-white ml-1">
            {(["editor", "preview"] as const).map((v) => (
              <button
                key={v}
                onClick={(e) => { e.stopPropagation(); onSetMobileView(v); }}
                className={`px-2.5 py-1.5 text-[11px] font-medium transition-all duration-200 capitalize border-r border-[#E0E0E0] last:border-r-0 ${
                  mobileView === v ? "bg-[#E3F2FD] text-[#1565C0]" : `${shell.textMuted} hover:text-[#616161] hover:bg-black/4`
                }`}
              >
                {v}
              </button>
        ...
        {/* Center — layout controls (lg+, md/latex only) */}
        {(mode === "markdown" || mode === "latex") && (
          <div className="hidden lg:flex items-center gap-2 font-mono" onClick={(e) => e.stopPropagation()}>
            <span className="text-[10px] text-[#636c76]">Margin</span>
            <input type="text" inputMode="decimal" value={templateLayout.marginMm}
              onChange={(e) => { const v = e.target.value; if (v === "" || /^\d*\.?\d*$/.test(v)) setTemplateLayout({ marginMm: v === "" ? 0 : Number(v) }); }}
            ...
            <input type="text" inputMode="decimal" value={templateLayout.paddingMm}
              onChange={(e) => { const v = e.target.value; if (v === "" || /^\d*\.?\d*$/.test(v)) setTemplateLayout({ paddingMm: v === "" ? 0 : Number(v) }); }}
              onKeyDown={(e) => { if (e.key === "ArrowUp" || e.key === "ArrowDown") e.preventDefault(); }}
              className="w-10 px-1.5 h-5.5 text-[11px] text-center bg-white text-[#616161] border border-[#E0E0E0] rounded-full outline-none focus:border-[#1976D2]/60 transition-colors"
            />
            <span className="text-[10px] text-[#8c959f]">mm</span>
            ...
            <input type="text" inputMode="decimal" value={templateLayout.lineHeight}
              onChange={(e) => { const v = e.target.value; if (v === "" || /^\d*\.?\d*$/.test(v)) setTemplateLayout({ lineHeight: v === "" ? 0 : Number(v) }); }}
              onKeyDown={(e) => { if (e.key === "ArrowUp" || e.key === "ArrowDown") e.preventDefault(); }}
        ...
        <div className="hidden sm:flex items-center gap-2">
          {/* Panel buttons — sm to lg */}
          <div className="flex lg:hidden items-center gap-1">
            {(["templates", "ats", "help"] as SidePanel[]).map((p) => (
              <button key={p} onClick={(e) => { e.stopPropagation(); onToggleSidePanel(p); }} className={panelBtnCls(p)} title={p}>
                <PanelBtnIcon panel={p} />
              </button>
            ))}
          </div>

          {/* Font selector */}
          <div className="hidden md:block" data-font-dropdown onClick={(e) => e.stopPropagation()}>
            <button
              ref={triggerRef}
              ...
              className={`flex items-center gap-1.5 px-3 h-7.5 rounded-full text-[11px] font-medium border bg-white transition-colors outline-none min-w-32.5 justify-between ${
                fontOpen ? "border-[#1976D2] text-[#212121]" : "border-[#E0E0E0] text-[#757575] hover:text-[#212121] hover:border-[#BDBDBD]"
              }`}
            >
              <span className="truncate">{selectedFontName}</span>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="10" height="10" className={`shrink-0 transition-transform duration-150 ${fontOpen ? "rotate-180" : ""}`}>
          ...
          {fontOpen && createPortal(
            <div data-font-dropdown style={{ position: "fixed", top: dropdownPos.top, right: dropdownPos.right }}
            ...
            >
              <div className="max-h-72 overflow-y-auto">
                <button onClick={() => { setFontId(""); onCloseAllAction(); setFontOpen(false); }}
                  className={`w-full px-3 py-1.5 text-[11px] text-left transition-colors ${!fontId ? "bg-[#E3F2FD] text-[#1565C0] font-medium" : "text-[#212121] hover:bg-[#F5F5F5]"}`}
                >
                  Default Font
                </button>
                {(["sans-serif", "display", "serif", "monospace"] as const).map((cat) => (
                  <div key={cat}>
                    ...
                    {FONT_OPTIONS.filter((f) => f.category === cat).map((f) => (
                      <button key={f.id} onClick={() => { setFontId(f.id); onCloseAllAction(); setFontOpen(false); }}
                        className={`w-full px-3 py-1.5 text-[11px] text-left transition-colors ${fontId === f.id ? "bg-[#E3F2FD] text-[#1565C0] font-medium" : "text-[#212121] hover:bg-[#F5F5F5]"}`}
                      >
                        {f.name}
                      </button>
                    ))}
          ...
          {/* Export — full */}
          <button onClick={(e) => { e.stopPropagation(); onExport(); }} disabled={exporting}
        ...
        {/* Export — xs only (compact) */}
        <button onClick={(e) => { e.stopPropagation(); onExport(); }} disabled={exporting}
          className={`flex sm:hidden items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold text-white transition-all duration-200 disabled:opacity-40 disabled:cursor-wait active:scale-95 ${exportGrad} ${exportGlow}`}
        >
          {exportIcon}
      ...
      {/* ══ Row 2: xs only — Editor/Preview toggle + Panel buttons ══ */}
      <div className="flex sm:hidden items-center justify-between pb-2 gap-2">
        <div className="flex items-center border border-[#E0E0E0] rounded-full overflow-hidden bg-white">
          {(["editor", "preview"] as const).map((v) => (
            <button key={v} onClick={(e) => { e.stopPropagation(); onSetMobileView(v); }}
              className={`px-3 py-1.5 text-[11px] font-medium transition-all duration-200 capitalize border-r border-[#E0E0E0] last:border-r-0 ${
                mobileView === v ? "bg-[#E3F2FD] text-[#1565C0]" : `${shell.textMuted} hover:text-[#616161] hover:bg-black/4`
              }`}
            >
              {v}
